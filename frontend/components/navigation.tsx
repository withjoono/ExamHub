...
export function Navigation() {
  ...
  const toggleMobileDropdown = (menuTitle: string) => {
    setActiveMobileDropdown(activeMobileDropdown === menuTitle ? null : menuTitle)
  }
        ...
        <div className="flex justify-between items-center h-16">
          ...
          <div className="hidden md:flex items-center space-x-8">
            {menuItems.map((menu) => (
              ...
              >
                <button className="flex items-center space-x-1 text-gray-700 hover:text-[#7b1e7a] py-2">
                      ...
                      <div className="py-1">
                        {menu.items.map((item) => (
                          <div key={item.name} className="relative">
                            {item.subItems ? (
                              <div
                                className="flex items-center justify-between px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 hover:text-[#7b1e7a] cursor-pointer"
                              ...
                              <Link
                                href={item.href}
                                className="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 hover:text-[#7b1e7a]"
                              >
                                {item.name}
                              </Link>
                            )}
                            ...
                            {item.subItems && activeSubDropdown === item.name && (
                              <div
                                className="absolute left-full top-0 pl-2 w-44 z-50"
                                  ...
                                  <div className="py-1">
                                    {item.subItems.map((subItem) => (
                                      <Link
                                        key={subItem.name}
                                        href={subItem.href}
                                        className="block px-4 py-3 text-sm text-gray-700 hover:bg-gray-100 hover:text-[#7b1e7a]"
                                      >
                                        {subItem.name}
                                      </Link>
                                    ))}
          ...
          <div className="hidden md:flex items-center space-x-4">
            <button className="bg-[#7b1e7a] hover:bg-[#5a1559] text-white px-4 py-2 rounded-md text-sm font-medium">
              무료체험
            </button>
            <button className="text-gray-700 hover:text-[#7b1e7a] text-sm font-medium">회원가입</button>
            <button className="text-gray-700 hover:text-[#7b1e7a] text-sm font-medium">로그인</button>
          </div>
          ...
          <div className="md:hidden border-t border-gray-200 bg-white">
            <div className="px-2 pt-2 pb-3 space-y-1">
              {menuItems.map((menu) => (
                <div key={menu.title}>
                  <button
                    ...
                    className="flex items-center justify-between w-full px-3 py-2 text-base font-medium text-gray-700 hover:text-[#7b1e7a] hover:bg-gray-50"
                  >
                    <span>{menu.title}</span>
                    <ChevronDown
                      className={`w-4 h-4 transition-transform ${
                        activeMobileDropdown === menu.title ? "rotate-180" : ""
                      }`}
                    />
                  </button>
                    ...
                    <div className="pl-4 space-y-1">
                      {menu.items.map((item) => (
                        <Link
                          key={item.name}
                          href={item.href}
                          className="block px-3 py-2 text-sm text-gray-600 hover:text-[#7b1e7a] hover:bg-gray-50"
              ...
              <div className="pt-4 space-y-2 border-t border-gray-200">
                <button className="w-full bg-[#7b1e7a] hover:bg-[#5a1559] text-white px-4 py-2 rounded-md text-sm font-medium">
                  무료체험
                </button>
                <div className="flex space-x-4">
                  <button className="flex-1 text-gray-700 hover:text-[#7b1e7a] text-sm font-medium py-2">
                    회원가입
                  </button>
                  <button className="flex-1 text-gray-700 hover:text-[#7b1e7a] text-sm font-medium py-2">
                    로그인
